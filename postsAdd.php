...
if(!empty($_POST))
{
	if(!empty($_POST['title']) &&  !empty($_POST['content']) )
	{
			...
			if(!empty($pictures['error']))
			{
				foreach ($pictures['error'] as $key => $error) 
				{
					if( $error === 0 )
					{
						...
						if($move === true)
						{
							$pictureUrl = "http://localhost/demo/web/images/{$name}";
		...
		$sqlupdate = "INSERT INTO `demo`.`posts` (`title`, `content`, `pictures`, `userID`, `createTime`) 
		VALUES ('{$title}', '{$content}', '{$pictures}', '{$userID}', '{$createTime}');";
